...
namespace EasyConnect.Protocols.Rdp
{
    ...
    public partial class RdpConnectionForm : BaseConnectionForm<RdpConnection>
    {
        ...
        public string Host
        {
            get //TODO should not use this output, because it cannot identify if user set the port
            {
                return _rdpWindow.Server + ':' + _rdpWindow.AdvancedSettings2.RDPPort;
            }
            ...
            set
            {
                ...
                _rdpWindow.Server = settings[0];

                int port;
                if (settings.Length > 0 && int.TryParse(settings[1], out port))
                {
                    _rdpWindow.AdvancedSettings2.RDPPort = port;
